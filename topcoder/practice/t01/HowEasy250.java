 ...
 * point problem. If the Average Word Length is equal to 4 or 5, the problem is
 * a 500 point problem. If the Average Word Length is greater than or equal to
 ...
 * 
 * Word Length - the number of letters in a Word. (NOTE: a period is NOT a
 * letter)
 * 
 ...
 * 
 * If problemStatement="523hi.", there are no Words, so the Average Word Length
 * is 0, and the method should return 250.
 * 
 ...
 * Returns: int Method
 * 
 * signature: int pointVal(String param0)
 * 
 ...
 * 
 * http://community.topcoder.com/tc?module=Static&d1=applet&d2=applet
 * 
 * @author Marco Lackovic <marco.lackovic@gmail.com>
 * @version 1.0, 24 Oct 2011
 */

public class HowEasy250 {

	private static final String WORD_REGEXP = "^[a-zA-Z]+(\\.)*$";

	private static final int PROBLEM_EASY_POINTS = 250;
	private static final int PROBLEM_EASY_LIMIT = 3;

	private static final int PROBLEM_MEDIUM_POINTS = 500;
	private static final int PROBLEM_MEDIUM_LIMIT = 5;

	private static final int PROBLEM_HARD_POINTS = 1000;

	private boolean isAWord(String s) {
		return s.matches(WORD_REGEXP);
	}
	...
	public int pointVal(String param0) {
		...
		int numberOfWords = 0;
		StringTokenizer param0tokenized = new StringTokenizer(param0);
		while (param0tokenized.hasMoreTokens()) {
			String token = param0tokenized.nextToken();
			if (isAWord(token)) {
				wordsTotalLength += wordLength(token);
				numberOfWords++;
			}
		}
		if (numberOfWords == 0) {
			return PROBLEM_EASY_POINTS;
		}
		int averageWordLength = wordsTotalLength / numberOfWords;
		if (averageWordLength <= PROBLEM_EASY_LIMIT) {
			return PROBLEM_EASY_POINTS;
		}
		if (averageWordLength <= PROBLEM_MEDIUM_LIMIT) {
			return PROBLEM_MEDIUM_POINTS;
		}
		return PROBLEM_HARD_POINTS;
	}
}